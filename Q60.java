...
Total Accepted: 51206 Total Submissions: 205983 Difficulty: Medium
...
public String getPermutation(int n, int k) {
    ...
    return getPermutation(list, k-1, ""); //read explain below: because index start from 0. but no 0th, as always k starts from 1st
}

private String getPermutation(ArrayList<Integer> list, int k, String progress){
    if(list.size() == 1){
        return progress+list.get(0); //REMEMBER STRING IS PASSED BY VALUE;
    //so you have to use return string in subrutin..; cannot use void and return the res
    }
    int grid = factorial(list.size()-1); //this means whichever is the first letter, it can have grid many of permutations; so later use k/grid to see how many grids k can afford, the result is also the index which is the first letter we should put
    int index = k/grid;  
    int digit = list.get(index);
    progress = progress + digit;
    list.remove(index); //index is the element number
