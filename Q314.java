...
public List<List<Integer>> verticalOrder(TreeNode root) {
    ...
    if (root == null) {
     ...
     //BFS: easier to understand; just use the cols queue to store the coordinate of each node
     //when polling one node from the node que, cols que also polls this node's corrdinate
     //and the corrdinate as a key; add it to map
   ...
   /* public class Solution {//BFS+treeMap solution
    public List<List<Integer>> verticalOrder(TreeNode root) {
        List<List<Integer>> res = new ArrayList<>();
        if(root == null) {
            return res;
